...
- `joint_tensor(tree, params)`: compute P ∈ ℝ^{κ^n} via recursive
  tensor contraction (cherry-folding).
- `felsenstein(tree, params, site)`: single-site likelihood via
  pruning; O(n·κ²).
- Validate: Felsenstein × site counts == full tensor sum for
...
- `svd_score(F, rank=κ)`: Σ_{i>κ} σ_i², Chifman–Kubatko score.
- Validate: when split is an edge of the true tree, rank ≤ κ
...
- **3×3 minors** of edge flattenings (Allman–Rhodes 2008 Theorem 4
  for 2-state; generators for 4-state).
- **Group-based invariants** in Fourier coordinates:
  - Hadamard transform on ℝ^{κ^n} (the Klein-four variant).
...
- Compute k-ranks of the three factor matrices in a rank-κ tensor
...
- External (optional): `ete3`, `biopython`, `dendropy` as reference
  parsers; `sagemath` for Gröbner verification (via subprocess).
